...
EPS_BUNDLE="${ROOT_DIR}/artifacts/iqpilot_eps_kompensator_private"
...
EPS_STATE_FILE="${INSTALL_ROOT}/.installed_eps_kompensator_manifest.sha256"
...
EPS_SENTINEL_BASE="${INSTALL_ROOT}/python/iqpilot_private/selfdrive/tuning/eps_kompensator"
...
if [ -f "${EPS_BUNDLE}/manifest.json" ]; then
  HAVE_BUNDLE=1
  EPS_HASH="$(manifest_hash "${EPS_BUNDLE}/manifest.json")"
  EPS_DST_HASH=""
  if [ -f "${EPS_STATE_FILE}" ]; then
    EPS_DST_HASH="$(cat "${EPS_STATE_FILE}" 2>/dev/null || true)"
  fi
  if ! module_present "${EPS_SENTINEL_BASE}" || [ "${EPS_HASH}" != "${EPS_DST_HASH}" ]; then
    NEED_INSTALL=1
  fi
fi
...
if [ -f "${EPS_BUNDLE}/manifest.json" ]; then
  python3 "${VERIFY_SCRIPT}" "${EPS_BUNDLE}"
  cp -a "${EPS_BUNDLE}"/. "${TMP_ROOT}/"
fi
